...
let make = (~value=?, ~onChange, ~accordionExpanded) => {
  <div className={Cn.make(["flex", "flex-row", "flex-1"])}>
    <MaterialUi.FormControl
      ...
      <MaterialUi.Select
        value={MaterialUi.Select.Value.string("rarity rank")} disabled=true fullWidth=true>
        <MaterialUi.MenuItem value={MaterialUi.MenuItem.Value.string("rarity rank")}>
          {React.string("rarity rank")}
        </MaterialUi.MenuItem>
      </MaterialUi.Select>
    </MaterialUi.FormControl>
    <MaterialUi.FormControl
